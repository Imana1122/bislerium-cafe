function ToggleIcons(itemId, isHovering) {
    ...
    if (isHovering) {
        ...
        currentTableData1.style.fontFamily = "";
        currentTableData2.style.fontFamily = "";
...
window.carouselSetup = function () {
    $('#carouselExampleIndicators').on('slide.bs.carousel', function (event) {
        var $carousel = $(this);
        var $slides = $carousel.find('.carousel-item');
        var currentIndex = $slides.index($('.carousel-item.active'));
        var nextIndex = $(event.relatedTarget).index();

        if (nextIndex === $slides.length - 1 && currentIndex === 0) {
